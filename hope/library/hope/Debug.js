...
// ::
//
//		- Debuggable.mixinTo(object, topic)   				to set up _debug(), _warn() and _error() methods
...
//				else if (entry[0] == ERROR) console.log.apply(console, entry.slice(1));
	...
	window.Debuggable = {
		...
		mixinTo : function(it, topic) {
			if (!topic && it.ClassName) topic = it.ClassName;
			if (!topic) throw "Debuggable.mixinTo("+it+"): must provide a topic.";
			...
			var debugMethods = {
				...
				_warn : function _warn() {
					...
					var args = combineArgs(WARN_STRING, this, arguments);
					debugLog[debugLog.length] = args;
				...
				_error : function _error() {
					...
					var args = combineArgs(ERROR_STRING, this, arguments);
					debugLog[debugLog.length] = args;
	...
	function combineArgs(level, target, args) {
